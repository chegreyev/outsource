...
# Employee take money
@bot.callback_query_handler(lambda callback: callback.data == 'employee_start_message__take_money')
def employeeTakeMoney(callback):
    bot.send_message(
        callback.message.chat.id,
        'Для какой цели вы бы хотели взять деньги ?'
    )
    bot.register_next_step_handler(
        callback.message,
        employeeTakeMoney__aim
    )

def employeeTakeMoney__aim(message):
    vopros.aim = message.text
    bot.send_message(
        message.chat.id ,
        'Какую сумму денег вы бы хотели взять ?'
    ...
    bot.register_next_step_handler(
        message,
        employeeTakeMoney__money
    )

def employeeTakeMoney__money(message):
    vopros.money = message.text
...
# Employee single vopros
@bot.callback_query_handler(lambda callback: callback.data == 'employee_start_message__vopros')
def employeeVoprosSingle(callback):
    bot.send_message(
        callback.message.chat.id,
        'Выберите какой тип вопроса',
        reply_markup=hr_main_vopros_markup
...
@bot.callback_query_handler(lambda callback: callback.data == 'employee_main_vopros_markup__single')
def employeeVoprosSingle_main(callback):
    bot.send_message(
        callback.message.chat.id,
        'Отлично , можете задать свой вопрос :'
    )
    ...
    bot.register_next_step_handler(
        callback.message,
        employeeVoprosSingle
    )


def employeeVoprosSingle(message):
    vopros.single_vopros = message.text
...
# Employee double Vopros
@bot.callback_query_handler(lambda callback: callback.data == 'employee_main_vopros_markup__double')
def employeeVoprosDouble_main(callback):
    bot.send_message(
        callback.message.chat.id,
        'Отлично , можете задать свой вопрос :'
    )
    ...
    bot.register_next_step_handler(
        callback.message,
        employeeVoprosDouble
    )


def employeeVoprosDouble(message):
    vopros.double_vopros = message.text
    ...
    bot.register_next_step_handler(
        message,
        employeeVoprosDouble_First
    )


def employeeVoprosDouble_First(message):
    vopros.double_vopros_first = message.text
    ...
    bot.register_next_step_handler(
        message,
        employeeVoprosDouble_Double
    )


def employeeVoprosDouble_Double(message):
    vopros.double_vopros_double = message.text
